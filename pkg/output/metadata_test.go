...
func TestFilterMetadata_AllFields(t *testing.T) {
	meta := &QueryMetadata{
		ExecutionTimeMilliseconds: 47,
		ScannedRecords:            42351,
		QueryID:                   "test-id",
	...
	result := FilterMetadata(meta, []string{"all"})
	if result != meta {
		t.Error("FilterMetadata with 'all' should return original pointer")
	}
}

func TestFilterMetadata_NilMeta(t *testing.T) {
	result := FilterMetadata(nil, []string{"queryId"})
	if result != nil {
		t.Errorf("FilterMetadata(nil, ...) should return nil, got %+v", result)
	}
}

func TestFilterMetadata_EmptyFields(t *testing.T) {
	meta := &QueryMetadata{QueryID: "test-id"}
	result := FilterMetadata(meta, nil)
	if result != meta {
		t.Error("FilterMetadata with nil fields should return original pointer")
	}
}

func TestFilterMetadata_SelectFields(t *testing.T) {
	meta := &QueryMetadata{
		ExecutionTimeMilliseconds: 47,
		ScannedRecords:            42351,
		ScannedBytes:              2982690,
	...
	filtered := FilterMetadata(meta, []string{"queryId", "scannedRecords", "analysisTimeframe"})

	// Selected fields should be populated
	if filtered.QueryID != "test-id" {
		t.Errorf("expected QueryID=test-id, got %q", filtered.QueryID)
	}
	if filtered.ScannedRecords != 42351 {
		t.Errorf("expected ScannedRecords=42351, got %d", filtered.ScannedRecords)
	}
	if filtered.AnalysisTimeframe == nil {
		t.Fatal("expected AnalysisTimeframe to be set")
	}
	if filtered.AnalysisTimeframe.Start != "2026-03-09T10:00:00Z" {
		t.Errorf("expected AnalysisTimeframe.Start=2026-03-09T10:00:00Z, got %q", filtered.AnalysisTimeframe.Start)
	}

	// Non-selected fields should be zero values
	if filtered.ExecutionTimeMilliseconds != 0 {
		t.Errorf("expected ExecutionTimeMilliseconds=0, got %d", filtered.ExecutionTimeMilliseconds)
	}
	if filtered.ScannedBytes != 0 {
		t.Errorf("expected ScannedBytes=0, got %d", filtered.ScannedBytes)
	}
	if filtered.ScannedDataPoints != 0 {
		t.Errorf("expected ScannedDataPoints=0, got %d", filtered.ScannedDataPoints)
	}
	if filtered.Sampled != false {
		t.Error("expected Sampled=false")
	}
	if filtered.DQLVersion != "" {
		t.Errorf("expected DQLVersion empty, got %q", filtered.DQLVersion)
	}
	if filtered.Query != "" {
		t.Errorf("expected Query empty, got %q", filtered.Query)
	}
	if filtered.CanonicalQuery != "" {
		t.Errorf("expected CanonicalQuery empty, got %q", filtered.CanonicalQuery)
	}
	if filtered.Timezone != "" {
		t.Errorf("expected Timezone empty, got %q", filtered.Timezone)
	}
	if filtered.Locale != "" {
		t.Errorf("expected Locale empty, got %q", filtered.Locale)
	}
	if filtered.Contributions != nil {
		t.Error("expected Contributions=nil")
	}
}

func TestFilterMetadata_NoMatchingFields(t *testing.T) {
	// When no fields match (e.g., all fields in the list are unrecognized),
	// the filtered struct should have all zero values.
	// Note: ParseMetadataFields now validates, so this scenario only arises
	// if FilterMetadata is called directly with unrecognized names.
	meta := &QueryMetadata{
		ExecutionTimeMilliseconds: 47,
		ScannedRecords:            42351,
		QueryID:                   "test-id",
	...
	filtered := FilterMetadata(meta, []string{"nonexistent", "alsoWrong"})

	if filtered.ExecutionTimeMilliseconds != 0 {
		t.Errorf("expected ExecutionTimeMilliseconds=0, got %d", filtered.ExecutionTimeMilliseconds)
	}
	if filtered.ScannedRecords != 0 {
		t.Errorf("expected ScannedRecords=0, got %d", filtered.ScannedRecords)
	}
	if filtered.QueryID != "" {
		t.Errorf("expected QueryID empty, got %q", filtered.QueryID)
	}
}
...
func TestFilterMetadata_YAMLOmitsUnselectedFields(t *testing.T) {
	meta := &QueryMetadata{
		ExecutionTimeMilliseconds: 47,
		ScannedRecords:            42351,
		ScannedBytes:              2982690,
	...
	filtered := FilterMetadata(meta, []string{"queryId", "scannedRecords"})
	...
	if !strings.Contains(yamlStr, "queryId: test-id") {
		t.Errorf("YAML should contain 'queryId: test-id', got:\n%s", yamlStr)
	}
	if !strings.Contains(yamlStr, "scannedRecords: 42351") {
		t.Errorf("YAML should contain 'scannedRecords: 42351', got:\n%s", yamlStr)
	}
...
func TestFilterMetadata_JSONOmitsUnselectedFields(t *testing.T) {
	meta := &QueryMetadata{
		ExecutionTimeMilliseconds: 47,
		ScannedRecords:            42351,
		ScannedBytes:              2982690,
	...
	// Selected fields must be present
	if !strings.Contains(jsonStr, `"queryId":"test-id"`) {
		t.Errorf("JSON should contain queryId, got:\n%s", jsonStr)
	}
	if !strings.Contains(jsonStr, `"scannedRecords":42351`) {
		t.Errorf("JSON should contain scannedRecords, got:\n%s", jsonStr)
	}

	// Unselected fields must be absent (omitempty zeros them out)
	for _, absent := range []string{
		`"executionTimeMilliseconds"`,
		`"scannedBytes"`,
		`"scannedDataPoints"`,
		`"sampled"`,
		`"dqlVersion"`,
		`"query"`, // won't match queryId because of the colon
		`"canonicalQuery"`,
		`"timezone"`,
		`"locale"`,
		`"analysisTimeframe"`,
...
// TestJSONPrinter_FilteredMetadataMap exercises the real JSONPrinter with
// the same map structure that printResults() builds in dql.go:
//
//	out := map[string]interface{}{"records": records, "metadata": filteredMeta}
//	printer.Print(out)
func TestJSONPrinter_FilteredMetadataMap(t *testing.T) {
	meta := &QueryMetadata{
		ExecutionTimeMilliseconds: 100,
		ScannedRecords:            5000,
		ScannedBytes:              1048576,
		QueryID:                   "json-printer-test-id",
		DQLVersion:                "V1_0",
		Timezone:                  "Europe/Vienna",
		Locale:                    "en",
	}

	filtered := FilterMetadata(meta, []string{"queryId", "executionTimeMilliseconds"})
...
// TestYAMLPrinter_FilteredMetadataMap exercises the real YAMLPrinter with
// the same map structure that printResults() builds in dql.go.
func TestYAMLPrinter_FilteredMetadataMap(t *testing.T) {
	meta := &QueryMetadata{
		ExecutionTimeMilliseconds: 200,
		ScannedRecords:            8000,
		ScannedBytes:              2097152,
		QueryID:                   "yaml-printer-test-id",
		DQLVersion:                "V1_0",
		Timezone:                  "UTC",
		Locale:                    "de",
		Sampled:                   true,
	...
	filtered := FilterMetadata(meta, []string{"queryId", "sampled", "analysisTimeframe"})
	...
	// Selected fields must be present
	if !strings.Contains(result, "queryId: yaml-printer-test-id") {
	...
	if !strings.Contains(result, "start: \"2026-03-09T10:00:00Z\"") && !strings.Contains(result, "start: 2026-03-09T10:00:00Z") {
		t.Errorf("YAML output should contain analysisTimeframe.start, got:\n%s", result)
	}
...
// TestFilterMetadata_SingleFieldSelection verifies that each of the 13
// metadata fields can be individually selected and only that field survives.
func TestFilterMetadata_SingleFieldSelection(t *testing.T) {
	meta := &QueryMetadata{
		ExecutionTimeMilliseconds: 47,
		ScannedRecords:            42351,
		ScannedBytes:              2982690,
	...
	tests := []struct {
		field      string
		checkJSON  string // a substring that must appear in the JSON
		absentJSON string // a substring that must NOT appear (to sanity-check isolation)
	}{
		{"executionTimeMilliseconds", `"executionTimeMilliseconds":47`, `"scannedRecords"`},
		{"scannedRecords", `"scannedRecords":42351`, `"executionTimeMilliseconds"`},
		{"scannedBytes", `"scannedBytes":2982690`, `"scannedRecords"`},
		{"scannedDataPoints", `"scannedDataPoints":100`, `"scannedBytes"`},
		{"sampled", `"sampled":true`, `"scannedDataPoints"`},
		{"queryId", `"queryId":"test-id"`, `"sampled"`},
		{"dqlVersion", `"dqlVersion":"V1_0"`, `"queryId"`},
		{"query", `"query":"fetch logs"`, `"dqlVersion"`},
		{"canonicalQuery", `"canonicalQuery":"fetch logs | limit 3"`, `"query":"fetch logs"`},
		{"timezone", `"timezone":"Z"`, `"canonicalQuery"`},
		{"locale", `"locale":"und"`, `"timezone"`},
		{"analysisTimeframe", `"analysisTimeframe"`, `"locale"`},
		{"contributions", `"contributions"`, `"analysisTimeframe"`},
	}

	for _, tt := range tests {
		t.Run(tt.field, func(t *testing.T) {
			filtered := FilterMetadata(meta, []string{tt.field})
			...
			if !strings.Contains(jsonStr, tt.checkJSON) {
				t.Errorf("selecting %q: expected JSON to contain %s, got: %s", tt.field, tt.checkJSON, jsonStr)
			}
			if strings.Contains(jsonStr, tt.absentJSON) {
				t.Errorf("selecting %q: expected JSON NOT to contain %s, got: %s", tt.field, tt.absentJSON, jsonStr)
			}
		})
	}
}
